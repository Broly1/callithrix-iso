...
old_buildiso=$(grep -oP "callithrixVersion='v\d{2}\.\d{2}\.\d{2}'" installation-scripts/build-the-iso.sh | grep -oP 'v\d{2}\.\d{2}\.\d{2}' | head -1)
old_profiledef=$(grep -oP 'callithrix-v\d{2}\.\d{2}\.\d{2}' archiso/profiledef.sh | grep -oP 'v\d{2}\.\d{2}\.\d{2}' | head -1)
...
# Replace entire callithrixVersion='...' line
sed -i "s|\(.*callithrixVersion='\)[^']*\('.*\)|\1$newversion\2|" installation-scripts/build-the-iso.sh

# Replace entire iso_label="callithrix-..." line
sed -i "s|^iso_label=\"callithrix-.*\"|iso_label=\"callithrix-$newversion\"|" archiso/profiledef.sh
